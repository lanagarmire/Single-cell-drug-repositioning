...
setwd("./")
#Drug Score ROC
VD<-read.table(file="./Validated_Drugs_COVID19.txt",header = F)
PD<-unique(c(as.character(VD[,1])))
PD.table<-data.frame(drug=PD,group="PD")
Rocs<-data.frame()
Cd<-readRDS("COVID19_drugscore_FDA_limma.rds")
...
Cd<-readRDS("COVID19_drugscore_FDA_seurat.rds")
Drugs<-unique(as.character(row.names(Cd)))
ND<-setdiff(Drugs,PD)
ND.table<-data.frame(drug=ND,group="ND")
Drug.table<-rbind(PD.table,ND.table)
...
Rocs=rbind(Rocs,Rocs.temp)
Cd<-readRDS("COVID19_drugscore_FDA_edgeR.rds")
Drugs<-unique(as.character(row.names(Cd)))
ND<-setdiff(Drugs,PD)
ND.table<-data.frame(drug=ND,group="ND")
Drug.table<-rbind(PD.table,ND.table)
...
Rocs=rbind(Rocs,Rocs.temp)
name<-paste0("COVID19_DEG.Drug_ROC.pdf")
...
plot.roc(E.roc,print.auc = T,print.auc.pattern=paste0("edgeR AUC=",round(E.roc$auc,2)),print.auc.x = .8,print.auc.y = .25,add = TRUE, col="#1c61b6")
dev.off()
...
PD<-unique(c(as.character(VD[,1])))
PD.table<-data.frame(drug=PD,group="PD")
path<-"CLUE/Single_cluster_COVID19/"
...
for(i in 2:ncol(Drug.data)){
  ...
  ScDrug.data<-readRDS("COVID19_drugs_all.rds")
  ...
  plot.roc(S.roc,print.auc = T,main=c,print.auc.x = .65,print.auc.y = .25,print.auc.pattern=paste0("Asgard AUC=",round(S.roc$auc,2)),col="#EE7785")
...
PD<-unique(c(as.character(VD[,1])))
PD.table<-data.frame(drug=PD,group="PD")
Cd<-readRDS("COVID19_all_drugscore_top4celltypes.rds")
Drugs<-unique(as.character(row.names(Cd)))
ND<-setdiff(Drugs,PD)
ND.table<-data.frame(drug=ND,group="ND")
Drug.table<-rbind(PD.table,ND.table)
Cd$drug=row.names(Cd)
Cd<-merge(Cd,Drug.table)
Cd<-subset(Cd,drug %in% Shared.drugs)
S.roc<-roc(Cd$group, Cd$Drug.therapeutic.score)
...
PD<-unique(c(as.character(VD[,1])))
PD.table<-data.frame(drug=PD,group="PD")
file<-"CLUE/COVID19_bulk/cs_n1x476251.gct"
...
Cd<-readRDS("COVID19_drugs_bulk_Drinsight.rds")
...
name<-paste0("COVID19_bulk_ROC_all.pdf")
pdf(file = name,width = 3.5,height = 3.5)
